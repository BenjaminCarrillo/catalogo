...
public class CatalogoServiceTest {
    ...
    void testAgregarProductoExistenteOk() {
        ...
        when(catalogoRepository.save(any(Catalogo.class)))
    ...
    void testAgregarProductoExistenteCatalogoInexistente() {
        when(catalogoRepository.findById(99L))
    ...
    void testAgregarProductoExistenteProductoInexistente() {
        ...
        when(productoRepository.findById(99L))
                .thenReturn(Optional.empty());
        assertThat(
                catalogoService.agregarProductoExistente(1L, 99L)
        ).isNull();
        verify(productoRepository, never())
                .save(any(Producto.class));
        verify(catalogoRepository, never())
                .save(any(Catalogo.class));
    }
